...
npr.seed(1)
N = 200 #RNN units
S = 10 #input, not used here
R = 1 #readout
B = 1 #batches per epoch
NK = 10 # number of conditions
kmax = 20 # max distance between input patterns
tmin, tmax = 10, 30 # max reaction time

Nepochs = 10000
doplot = True
Nepochsplot = Nepochs / 5 #plot output every certain number of epochs
dt = .1
T = 40
period = 10
NT = int(T/dt)
...
for i in range(NK):
  ...
  rtarg[i, int(tstar[i]/dt):, 0, 0] = 0.1
...
for ei in range(Nepochs):
    ...
    loss = torch.sum(torch.pow(r - rtarg[idx],2)) / (NT*B)
    ...
    if doplot and (ei == 0 or ei%Nepochsplot == 0 or ei%(Nepochs - 1) == 0):
        plt.clf()
        #plt.subplot(211)
...
for ei in range(Nepochs):
    ...
    if not len(np.argwhere(r[:, 0, 0].detach().numpy() > 0.08)) == 0:
      responsetime.append(np.argwhere(r[:, 0, 0].detach().numpy() > 0.08)[0] / (10*tmax))
    else:
      responsetime.append(0)
    #responsetime.append(np.argmax(r[:, 0, 0].detach().numpy()) / (10*tmax))

p = plt.figure(figsize = (6,6))
sns.regplot(distances, responsetime, marker='o', scatter_kws={'s':20}, color='red')
ax = plt.gca()
plt.xlim(0, 1)
plt.ylim(0, 1)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.xlabel('Distance between latents', fontsize=18)
plt.ylabel('Reaction time', fontsize=18)
plt.tight_layout()
p.savefig('rnncorr.png', dpi=200)
